...
# Fill missing values in categorical features with default values
features['Teacher_Quality'].fillna(value='Low', inplace=True)
features['Parental_Education_Level'].fillna(value='College', inplace=True)
features['Distance_from_Home'].fillna(value='Far', inplace=True)
...
rf_model = RandomForestRegressor(n_estimators=100, random_state=42)  # Initialize Random Forest model with 100 trees
...
opt = Adam(learning_rate=0.0001)  # Adam optimizer with a learning rate of 0.0001
model.compile(
...
model.fit(features_train, score_train, epochs=100, batch_size=32, verbose=2)  # Train for 100 epochs
